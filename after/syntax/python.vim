if g:colors_name == 'coop'
	" syn keyword pythonBuiltin self
	"syn keyword pythonComment self
	syn keyword pySelf self
	syn keyword pyPrint print
	syn keyword pyDef def
	syn keyword pyClass class
	syn keyword pythonTodo IDEA FIX

	syn keyword pyParen \( \)
	"syn region pyParen matchgroup=
	"
	"syn match Foo /([^(]*\%#.*)/
	...
	hi level16c ctermfg=red guifg=grey40
	...
	hi level17c ctermfg=red guifg=orange
	syn region level17 matchgroup=level17c start=/\[/ end=/\]/ contains=TOP,level17,NoInParens
endif

if g:colors_name == 'farout'
	syn keyword pythonBuiltin format
endif

if g:colors_name == 'benlight'
	syn keyword Function format self
	hi FoldColumn guifg=grey30 guibg=#141414
	hi LineNr guifg=grey50 guibg=#141414
